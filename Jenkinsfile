node {
    ...
    stage('Deploy image') {
        ...
        withCredentials([usernamePassword(credentialsId: 'deploy', passwordVariable: 'C_PASSWORD', usernameVariable: 'C_USER')]) {
            sh "ssh -o StrictHostKeyChecking=no -t arcsin@firexproxy.com -p2244 'echo ${environments} > .env && docker stop xscraper || true && docker login -u ${C_USER} -p ${C_PASSWORD} docker.firexproxy.com && docker pull docker.firexproxy.com/firexproxy/x.scraper:${env.BUILD_ID} && docker run -d --rm -p 60000:4040 --env-file .env --name xscraper docker.firexproxy.com/firexproxy/x.scraper:${env.BUILD_ID} && rm -f ~/.env'"
        }
    }
}
